...
const MenuOption = ({ icon, id, isActive, onClick, tooltip }: MenuOptionProps) => (
  <Tooltip content={tooltip} id={id} className="first-of-type:*:first:rounded-l-lg last-of-type:*:first:rounded-r-lg">
    <button
      type="button"
      className={cn(
...
export const MenuBar = ({
  ...
  return (
    <nav className={cn('bg-ob-base-100 flex rounded-lg shadow-xs transition-colors', className)} ref={menuRef}>
